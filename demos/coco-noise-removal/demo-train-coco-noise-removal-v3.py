...
def prepare(dataset, epochs, batch_size, input_shape, output_shape):
    ...
    #      ds.resize(u.mul_shape(output_shape, 4)), stream)
    #  stream = ds.stream(ds.apply_to_y(lambda x: (x, x, x)), stream)
    stream = ds.stream(ds.apply_to_y(resize_all(output_shape)), stream)
    #  stream = ds.stream(u.check, stream)
    #  lambda xn: ds.resize(output_shape))), stream)
...
model.compile(
    ...
    loss=[binary_focal_loss(), binary_focal_loss(), binary_focal_loss()],
