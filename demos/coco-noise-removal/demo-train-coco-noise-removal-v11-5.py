...
def mul_layer(back_layer, last):
    ...
    to1024 = Conv2D(
        int(back.shape[3]), (3, 3), activation='elu', padding='same')(upsample)
    to1024 = Conv2D(int(back.shape[3]), (1, 1), activation='elu')(to1024)
    mul = layers.Multiply()([back, to1024])
    l1x1 = Conv2D(1, (1, 1), activation='sigmoid')(mul)
    return mul, l1x1
