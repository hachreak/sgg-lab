...
from sgg_lab.metrics import fmeasure, precision, recall
...
def prepare(dataset, epochs, batch_size, input_shape, output_shape):
    ...
    batch = ds.stream(ds.apply_to_y(
        lambda y: [v for v in np.repeat(y, 2, axis=0)]), batch)
...
def get_model(input_shape=None, input_tensor=None):
    model = resnet50.ResNet50(
        include_top=False, input_shape=input_shape, input_tensor=input_tensor)
